...
from common.common_utils import batch_rpop, client, async_client_proxy, handle_headers
...
class ScopusUcc():
    ...
    def __init__(self, journal, start_year):
        ...
        self.host = "www-scopus-com.ucc.ilibs.cn"
...
User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{random.randint(106, 112)}.0.0.0 Safari/537.36
''')
    ...
    async def product_do_search(self):
        url = "https://www.scopus.com/search/submit/advanced.uri"
        form_data = {
            ...
            async with sem:
                ...
                if resp.status_code in [200, 201, 301, 302]:
                    if "error" in resp.text:
                        with open(r"./err_page/err_ucc_search.html", "w", encoding="utf8") as f:
                            f.write(resp.text)
    ...
    async def product_result_list(self, page, search_data):
        ...
        try:
            try:
                ...
                pass
            async with sem:
                resp = await async_client_proxy.get(url, params=params, headers=handle_headers(self.headers), follow_redirects=True,
                ...
                if resp.status_code in [200, 201]:
                    ...
                    try:
                        result_count = int(html.xpath('//*[@id="searchResFormId"]/div[1]/div/header/h1/span[@class="resultsCount"]/text()')[0].strip("\n").strip(" ").replace(",", ""))
                    ...
                    with open(r"./err_page/err_ucc_req_list.html", "w", encoding="utf8") as f:
                        f.write(resp.text)
                    raise Exception(f"err_status_code {resp.status_code}")
        ...
        try:
            ...
            async with sem:
                ...
                if resp.status_code in [200, 201]:
                    ...
                    with open(r"./err_page/err_scopus_ucc_detail.html", "w", encoding="utf8") as f:
                        f.write(resp.text)
                    raise Exception(f"err_status_code {resp.status_code}")
    ...
    def consumer_doc_detail(self, link):
        ...
        log.info(f"eid {eid}")
        url = f"http://{self.host}/doc-details/documents/{eid}"
        try:
...
def scopus_product_main():
    name = "journals:scopus_ucc"
    while True:
        journal = bytes.decode(_redis.redis_db.lpop(name))
        try:
            start_year = 2018
            scopus = ScopusUcc(journal, start_year)
            scopus.product_main()
            del scopus
