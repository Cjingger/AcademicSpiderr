...
from common.common_utils import compile_js, batch_rpop, local2utc, handle_headers, async_client, sslgen
...
class EIpzd(object):
    ...
    def __init__(self, journal, start_year, _search_type, search_id):
        ...
        self.host = "www.engineeringvillage.zd.hggfdd.top"
        self.host_url = "http://www.engineeringvillage.zd.hggfdd.top/"
        self.referer = f"{self.host_url}search/quick.url"
        self.headers = headersChange(f'''accept: application/json, text/javascript, */*; q=0.01
accept-encoding: gzip, deflate, br
accept-language: zh-CN,zh;q=0.9
cache-control: no-cache
content-type: application/x-www-form-urlencoded
origin: http://www.engineeringvillage.zd.hggfdd.top
pragma: no-cache
referer: {self.referer}
Host: {self.host}
user-agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{random.randint(109, 121)}.0.0.0 Safari/537.36
...
''')
    ...
    async def search_submit(self, start_year: int):
        endYear = str(datetime.datetime.now().year) if start_year < datetime.datetime.now().year else start_year
        submit_url = f"{self.host_url}search/submit.url"
        # form_data = {
        #     "usageOrigin": "searchform",
        #     "usageZone": self.search_type_map[self._search_type],
        #     "editSearch": "",
        ...
        form_data = {
            ...
            "updatesNo": "1",
            "language": "NO-LIMIT",
            "doctype": ["NO-LIMIT", "IP", "BK", "CH", "CA", "CP", "DS", "ED", "ER", "JA", "NO", "PA", "PP", "RC", "RR", "TB", "ST"],
            "sort": "relevance",
            "treatmentType": "NO-LIMIT",
            "searchStartTimestamp": str(int(time.time() * 1e3))
        }
        ...
        try:
            async with sem:
                ...
                if resp.status_code in [200, 201]:
                    ...
                    if int(resp_data["pagenav"]["resultscount"]) > 100 and len(resp_data["results"]) < 100:
                        ...
                        for r in resp_data["results"]:
                            ...
                            try:
                                ...
                                link = f"{self.host_url}app/doc/?docid={doc_id}&pageSize=100&index={doc_index}&searchId={search_id}&resultsCount={str(resp_data['pagenav']['resultscount'])}&usageZone=resultslist&usageOrigin=searchresults&searchType=Quick"
                                pub_year = int(r["yr"])
                                d = {
                                    "title": title,
                                    "link": link,
                                ...
                                await aio_redis.aredis_db.lpush(f"EI-bio-medical:{self.journal}", json.dumps(d))
                    ...
                    with open(r"./err_page/err_EI_nmg_search_quick_index.html", "w", encoding="utf8") as f:
                        f.write(resp.text)
    ...
    async def search_quick_page(self, search_id, result_count, page: int):
        ...
        try:
            url = f"https://www.engineeringvillage.com/search/results/quick.url?navigator=NEXT&SEARCHID={search_id}&database=8388609&angularReq=true&isFullJsonResult=false&usageOrigin=searchresults&COUNT={str(count)}&usageZone=nextpage&_{str(int(time.time() * 1e3))}"
            async with sem:
                seed = "".join(random.sample(string.ascii_lowercase + string.digits, random.randint(12, 12)))
                _async_c = httpx.AsyncClient(timeout=_timeout, limits=_limit, follow_redirects=True, verify=False,
                                             http2=True)
                ...
                if resp.status_code in [200, 201]:
                    ...
                    with open(r"./err_page/err_EI_pzd_search.html", "w", encoding="utf8") as f:
    ...
    async def search_doc_detail(self, search_id, search_data):
        ...
        try:
            ...
            if has_val(self._cookie, "EISESSION"):
                ...
                seed = "".join(random.sample(string.ascii_lowercase + string.digits, random.randint(12, 12)))
                async with sem:
                    _async_c = httpx.AsyncClient(timeout=_timeout, limits=_limit, follow_redirects=True, verify=False, http2=True)
            ...
            await aio_redis.aredis_db.lpush(f"EI-bio-medical:{journal}", json.dumps(d))
    ...
    async def async_consumer_main(self):
        ...
        keys = await aio_redis.aredis_db.keys(pattern='*EI-bio-medical:*')
        if len(keys) == 0:
            raise Exception("生产者队列已取完")
        keys = [bytes.decode(_).replace("EI-bio-medical:", "") for _ in keys]
        for k in keys:
            log.debug(f"pop key {k}")
            self.journal = k
            p = await aio_redis.aredis_db.pipeline()
            try:
                _datas = await batch_rpop(p, f"EI-bio-medical:{k}", random.randint(15, 20))
                _datas = _datas[1]
                if len(_datas) != 0:
                    ret = await self.search_submit(datetime.datetime.now().year)
                    if not ret:
                        _tasks = [aio_redis.back_in_redis(f"EI-bio-medical:{k}", json.dumps(literal_eval(bytes.decode(_data)))) for _data in _datas]
...
def EI_pzd_product_main(start_year):
    name = "journals:EI-bio-medical"
    search_type = "Quick"
    search_id = ""
    while True:
        ...
        try:
            for year in range(start_year, datetime.datetime.now().year + 1):
                EI = EIpzd(journal, year, search_type, search_id)
